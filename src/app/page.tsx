...
export default function HomePage() {
  ...
  const loadTasksFromSupabase = useCallback(async () => {
    ...
    try {
      ...
      console.log('[loadTasksFromSupabase] Tasks to be set in state:', fetchedTasks.map(t => ({id: t.id, completed: t.completed, description: t.description })));
  ...
  const handleToggleComplete = useCallback(async (id: string) => {
    ...
    try {
      ...
      await loadTasksFromSupabase(); // Ensure UI consistency even on caught error
    }
  }, [tasks, toast, user, setTasks, loadTasksFromSupabase]);
